...
class Pep(DataOceanModel):
    fullname = models.CharField("повне ім'я", max_length=120)
    fullname_eng = models.CharField("повне ім'я англійською", max_length=120)
    fullname_transcriptions_eng = models.CharField('варіанти повного імені англійською')
    ...
    last_workplace_eng = models.CharField('останнє місце роботи англійською', max_length=512,
                                          null=True)
    info = models.CharField('додаткова інформація', null=True)
    info_eng = models.CharField('додаткова інформація англійською', null=True)
    sanctions = models.CharField('відомі санкції проти особи', null=True)
    sanctions_eng = models.CharField('відомі санкції проти особи англійською', null=True)
    criminal_record = models.CharField('відомі вироки щодо особи', null=True)
    criminal_record_eng = models.CharField('відомі вироки щодо особи англійською', null=True)
    assets_info = models.CharField('дані про активи', null=True)
    assets_info_eng = models.CharField('дані про активи англійською', null=True)
    criminal_proceedings = models.CharField('відомі кримінальні провадження щодо особи', null=True)
    criminal_proceedings_eng = models.CharField('відомі кримінальні провадження щодо особи англійською',
                                                null=True)
    wanted = models.CharField('перебування в розшуку', null=True)
    wanted_eng = models.CharField('перебування в розшуку англійською', null=True)
...
class PepRelatedPerson(DataOceanModel):
    pep = models.ForeignKey(Pep, related_name='related_persons',
...
class PepDeclaration(DataOceanModel):
    pep = models.ForeignKey(Pep, related_name='declarations',
...
class CompanyLinkWithPep(DataOceanModel):
    ...
    link_type = models.CharField("тип зв'язку із публічним діячем", max_length=50,
                                 null=True)
    link_type_eng = models.CharField("тип зв'язку із публічним діячем англійською",
                                     max_length=50, null=True)
    link_start_date = models.DateField("дата виникнення зв'язку із публічним діячем", null=True)
    link_end_date = models.DateField("дата припинення зв'язку із публічним діячем", null=True)
    link_confirmation_date = models.DateField("дата підтвердження зв'язку із публічним діячем",
                                              null=True)
